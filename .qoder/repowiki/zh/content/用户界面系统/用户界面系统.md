...
- 改进的侧边栏树形导航：采用 HTML `<details>/<summary>` 实现，绕过 Streamlit 展开器嵌套限制，支持文件元数据指纹缓存优化
- 增强的主题系统：支持暗色/亮色双主题切换
- 完善的用户认证服务：基于PBKDF2-SHA256的密码哈希存储
- 增强的历史记录管理：支持多会话管理和聊天记录导出
- **新增** HTML `<details>/<summary>` 实现：侧边栏文件树采用原生HTML标签实现，支持无限层级嵌套
- **新增** 文件元数据指纹缓存：基于目录指纹的缓存机制，避免每次渲染读取全部文件
...
**更新** 本版本引入了全面增强的管理员面板、重新设计的登录界面、改进的侧边栏树形导航和增强的主题系统等重要UI改进。特别新增了基于 HTML `<details>/<summary>` 的侧边栏实现和文件元数据指纹缓存机制，显著提升了性能和用户体验。
...
C["ui/sidebar.py<br/>侧边栏导航<br/>HTML details/summary 树形导航"]
...
N["rag/loader.py<br/>文件元数据获取<br/>指纹缓存机制"]
end
A --> B
A --> C
A --> D
...
- 侧边栏导航：包含动态Logo、版本信息展示、主题切换、会话管理、知识库文档树形浏览、历史记录与系统状态。**新增** 采用 HTML `<details>/<summary>` 实现，支持无限层级嵌套和文件元数据指纹缓存
...
- **新增** 树形文档浏览：采用 HTML `<details>/<summary>` 实现，支持知识库文档的树形折叠展示，增强文档导航体验
- **新增** 文件元数据指纹缓存：基于目录指纹的缓存机制，避免每次渲染读取全部文件，提升性能

**更新** 新增管理员面板、树形文档浏览、增强的用户认证服务、HTML `<details>/<summary>` 实现和文件元数据指纹缓存等核心组件。
...
## 架构总览
下图展示了从用户交互到检索生成的端到端流程，包括状态管理、事件触发与流式渲染，以及新增的管理员面板、树形文档浏览机制和 HTML `<details>/<summary>` 实现。
...
participant LO as "文件加载(rag/loader.py)"
U->>S : 打开页面
S->>L : 渲染登录页(双模式认证)
U->>L : 输入用户名/密码或注册
L->>AU : 用户认证/注册
...
S->>SB : 渲染侧边栏(HTML details/summary 树形导航)
SB->>LO : 获取文件元数据(指纹缓存)
LO-->>SB : 返回文件元数据
SB-->>S : 切换主题/会话/导航
S->>CP : 渲染聊天面板
...
- 功能要点
  ...
  - 会话管理：新建、切换、清空会话
  - **新增** 知识库文档树形浏览：采用 HTML `<details>/<summary>` 实现，支持关键词搜索、目录折叠展开
  - **新增** 文件元数据指纹缓存：基于目录指纹的缓存机制，避免每次渲染读取全部文件
  - 历史记录：按日期展开，显示部分消息
...
- 交互与事件
  ...
  - **新增** 搜索功能：支持按标题和路径搜索文档
- 自定义选项
  - 标题、副标题、公司名、默认主题由配置项控制
  - **新增** `company_url` 字段支持品牌化部署
- 无障碍建议
  - 为按钮添加工具提示与语义化标签
  - 展开面板使用 HTML details/summary 并提供折叠/展开状态提示

**更新** 新增版本信息显示、知识库文档树形浏览、文件元数据指纹缓存和增强的版权信息功能。
...
- 技术实现
  ...
  - `_get_dir_fingerprint()`: 计算目录指纹（基于文件数量+最新修改时间）
- 自定义选项
  - 通过配置文件控制知识库目录和分块参数
  - 支持自定义文档目录位置
- 无障碍建议
...
### 文件元数据指纹缓存组件
- 功能要点
  - **新增** 目录指纹计算：基于文件数量和最新修改时间生成唯一指纹
  - **新增** 缓存失效机制：仅在目录指纹变化时重新扫描文件元数据
  - **新增** 性能优化：避免每次渲染读取全部文件，显著提升侧边栏加载速度
  - **新增** 稳定性保障：指纹计算包含异常处理，确保缓存机制稳定可靠
- 技术实现
  - `_get_dir_fingerprint()`: 计算目录指纹（文件数量+最新修改时间）
  - `_load_files_meta_cached()`: 带缓存的文件元数据加载
  - 缓存键：`sidebar_files_meta` 和 `sidebar_files_fingerprint`
- 自定义选项
  - 支持自定义缓存键名
  - 可配置指纹计算逻辑
- 无障碍建议
...
**新增** 专门的文件元数据指纹缓存组件，提供高效的侧边栏性能优化。
...
- **新增** 树形文档浏览性能
  ...
  - 无限层级嵌套支持，突破传统 Streamlit expander 嵌套限制
- **新增** 管理员面板性能
...
- **新增** 文件元数据缓存性能
  - 目录指纹缓存避免每次渲染读取全部文件
  - 缓存键使用 session_state 存储，跨页面保持有效
  - 异常处理确保缓存机制稳定性

**更新** 新增树形文档浏览、管理员面板、版本信息和文件元数据指纹缓存的性能考量。
...
- [rag/__init__.py:3](file://rag/__init__.py#L3)
- [rag/loader.py:217-250](file://rag/loader.py#L217-L250)
...
- **新增** 树形文档浏览异常
  ...
  - **新增** HTML `<details>/<summary>` 嵌套问题：确认目录层级不超过限制
- **新增** Logo显示问题
  - 现象：Logo显示为文本符号而非图片
...
- **新增** 文件元数据缓存问题
  - 现象：侧边栏文档列表不更新
  - 处理：检查目录指纹缓存是否正确失效，确认文件修改时间变化

**更新** 新增管理员面板、树形文档浏览、登录认证、版本信息、HTML `<details>/<summary>` 实现和文件元数据缓存相关的故障排查指南。
...
**更新** 本版本显著增强了系统功能完整性，通过全面的管理员面板、重新设计的登录界面、改进的侧边栏树形导航和增强的主题系统，为企业用户提供更加专业和统一的品牌体验。同时新增了用户认证服务、树形文档浏览、版本信息显示、HTML `<details>/<summary>` 实现和文件元数据指纹缓存等核心功能，提供了更完善的系统管理能力、更好的用户体验和更高的性能表现。
...
- 侧边栏组件
  - 属性：默认主题、管理员标识、系统状态、动态Logo、版本信息、版权信息、文档树配置、HTML `<details>/<summary>` 实现、文件元数据缓存配置
  - 事件：主题切换、导航切换、会话切换/新建、清除会话、断开连接、文档搜索、目录展开/折叠
- 聊天面板
...
- **新增** 树形文档浏览组件
  - 属性：文档目录配置、搜索配置、缓存配置、Doxygen兼容配置、HTML `<details>/<summary>` 实现
  - 事件：目录展开/折叠、文件点击、关键词搜索、元数据显示
- **新增** 文件元数据指纹缓存组件
  - 属性：目录指纹计算配置、缓存键配置、异常处理配置
  - 事件：目录指纹计算、缓存失效、文件元数据加载
- **新增** Logo渲染组件
...
**更新** 新增管理员面板、用户认证服务、树形文档浏览、文件元数据指纹缓存、Logo渲染、版本信息和错误详情组件的属性和事件清单。
...
- **新增** 树形文档浏览定制
  ...
  - **新增** HTML `<details>/<summary>` 定制：调整展开/折叠样式和行为
- **新增** 文件元数据指纹缓存定制
  - 配置缓存键名：修改 `sidebar_files_meta` 和 `sidebar_files_fingerprint`
  - 自定义指纹计算逻辑：调整 `_get_dir_fingerprint()` 方法
  - 配置缓存失效条件：调整指纹比较逻辑
- **新增** Logo定制
...
**更新** 新增管理员面板、用户认证、树形文档浏览、文件元数据指纹缓存、Logo、版本信息、错误信息和标题栏的定制指南。
