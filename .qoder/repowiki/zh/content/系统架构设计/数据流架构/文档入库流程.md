...
## 更新摘要
**变更内容**
- 更新向量嵌入部分，反映 ChineseEmbeddingFunction 提供 L2 归一化向量的特性
- 更新距离计算和相关性评分的解释方式
- 更新配置中的距离阈值说明
- 更新 RAG 检索链中距离阈值的使用逻辑
...
**更新** 本版本特别关注嵌入向量归一化的影响，ChineseEmbeddingFunction 现在提供 L2 归一化的向量，这改变了距离计算和相关性评分的解释方式。
...
EMBED["rag/embeddings.py<br/>中文嵌入函数<br/>L2 归一化"]
...
VS->>EmbedFn : 为每个 chunk 计算 L2 归一化向量
EmbedFn-->>VS : 向量列表平方 L2 距离 ∈ [0,4]
...
- [vectorstore.py:89-120](file://rag/vectorstore.py#L89-L120)
...
- 环境优化：设置 HF_ENDPOINT 为国内镜像，加速模型下载
- **重要更新** L2 归一化：输出向量经过 L2 归一化，位于单位球面上，平方 L2 距离范围为 [0, 4]

**更新** ChineseEmbeddingFunction 现在提供 L2 归一化的向量，这意味着：
- 向量位于单位球面上
- 平方 L2 距离 ∈ [0, 4]
- 可使用合理的距离阈值（~1.5）过滤低相关度结果
- 余弦相似度与平方 L2 距离之间存在明确的数学关系
...
ChineseEmbeddingFunction : 输出向量经 L2 归一化
ChineseEmbeddingFunction : 平方 L2 距离 ∈ [0,4]
```
...
**图表来源**
- [ingest.py:25-58](file://scripts/ingest.py#L25-L58)
- [config.yaml:38-45](file://config.yaml#L38-L45)
...
**更新** 配置中的距离阈值现在基于 L2 归一化向量的距离范围 [0, 4]：
...
**更新** RAG 检索链现在使用 L2 归一化向量的距离阈值：

- 向量检索使用平方 L2 距离，范围 [0, 4]
- distance_threshold 默认值 1.5 过滤低相关度结果
- 低置信度检测：top_distance > 2.0（对应余弦相似度≈0）表示匹配度接近随机
...
- [vectorstore.py:181-217](file://rag/vectorstore.py#L181-L217)
...
- 模型加载缓慢或失败
  ...
  - 处理：确保 HF_ENDPOINT 设置为国内镜像；若离线部署，提供 local_path
  - 参考：[embeddings.py:9-10](file://rag/embeddings.py#L9-L10), [embeddings.py:62-75](file://rag/embeddings.py#L62-L75)
- 入库无结果
  - 现象：向量库统计为 0 或检索不到
...
- 检索质量不佳
  ...
  - **更新** 距离阈值现在基于 L2 归一化向量：默认 1.5，对应余弦相似度 ≈ 0.25
  - 参考：[config.yaml:42-44](file://config.yaml#L42-L44), [chain.py:290](file://rag/chain.py#L290)
- 前端状态异常
  - 现象：RAGChain 未就绪或报错
  - 处理：检查 preload 状态；查看后台线程日志；必要时 reset
  - 参考：[preload.py:22-49](file://rag/preload.py#L22-L49), [app.py:32-41](file://app.py#L32-L41)
- **新增** L2 归一化相关问题
  - 现象：距离阈值设置不合理导致检索结果过多或过少
  - 处理：根据实际需求调整 distance_threshold（默认 1.5，范围 [0,4]）
  - 参考：[embeddings.py:77-87](file://rag/embeddings.py#L77-L87), [config.yaml:42-44](file://config.yaml#L42-L44)

**章节来源**
- [loader.py:101-118](file://rag/loader.py#L101-L118)
- [embeddings.py:9-10](file://rag/embeddings.py#L9-L10)
- [embeddings.py:62-75](file://rag/embeddings.py#L62-L75)
- [ingest.py:35-41](file://scripts/ingest.py#L35-L41)
- [config.yaml:42-44](file://config.yaml#L42-L44)
- [chain.py:290](file://rag/chain.py#L290)
- [preload.py:22-49](file://rag/preload.py#L22-L49)
- [app.py:32-41](file://app.py#L32-L41)

## 结论
本系统通过清晰的分层设计与模块化实现，完成了从多格式文档到向量存储的高效入库流程。loader 提供稳健的分块与元数据提取，embeddings 与 vectorstore 实现了可扩展的向量化与持久化，ingest 脚本提供了便捷的批量入库能力。结合 RAGChain 的混合检索与重排序策略，系统在准确性与性能上取得平衡。

**更新** 本次更新特别关注了 L2 归一化向量的影响，这一改进带来了以下优势：
- 统一的向量尺度，便于距离阈值设定和相关性评分解释
- 明确的平方 L2 距离范围 [0, 4]，提供直观的相似度度量
- 无需额外的向量归一化步骤，提升计算效率
- 更好的检索质量控制，通过合理的距离阈值过滤低相关度结果

建议在生产环境中关注模型加载、网络与磁盘 I/O 的优化，并定期清理与统计向量库，以维持长期稳定运行。
...
- 配置项说明（节选）
  ...
  - **更新** vectorstore.distance_threshold：基于 L2 归一化向量的距离阈值，默认 1.5
- 测试覆盖
  - loader：分块、标题提取、元数据枚举
  - embeddings：初始化与延迟加载、L2 归一化验证
