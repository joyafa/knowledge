...
## 更新摘要
**变更内容**
- 更新了安装器构建脚本的参数说明，从 `--skip-models` 改为 `--include-models`
- 新增了默认行为变更说明：不打包模型，改为运行时自动下载
- 更新了 Windows 构建脚本简化说明
- 补充了模型自动下载机制的技术细节
- 更新了离线安装包制作的相关说明
...
**更新** 本版本重点介绍了新的安装器构建脚本重构，包括参数变更和默认行为调整。
...
L -.-> J
M["PyInstaller 规格<br/>knowledge_app.spec"] -.-> C
```
...
**更新** 新版本强调了 `--include-models` 参数的作用和默认行为的改变。
...
## 结论
通过统一的安装包构建与离线部署方案，系统管理员可在不同平台快速完成安装、配置、入库与启动。**更新** 新版本的重构显著改进了安装体验：默认不打包模型，运行时自动下载，大幅减小安装包体积；同时提供了更灵活的 `--include-models` 参数控制，满足不同部署场景的需求。建议在生产环境中结合防火墙放行、开机自启与日志监控，确保服务稳定运行。
