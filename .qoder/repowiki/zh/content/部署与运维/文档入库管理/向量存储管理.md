...
## 更新摘要
**变更内容**
- 更新了 VectorStore 类的批量入库功能，增加了智能批次处理机制
- 新增了4000个文档的批量处理能力，避免ChromaDB的默认批次限制
- 优化了删除过程的批处理操作，支持5000个文档的批量删除
- 增强了大规模数据处理的性能和稳定性
...
**更新** 本次更新重点介绍了VectorStore类的智能批次处理机制，该机制能够自动处理大量文档的批量入库，支持最多4000个文档的单次批量处理，有效避免ChromaDB的默认批次限制。
...
**图表来源**
- [rag/vectorstore.py:24-230](file://rag/vectorstore.py#L24-L230)
...
- [scripts/ingest.py:25-67](file://scripts/ingest.py#L25-L67)
...
**章节来源**
- [rag/vectorstore.py:1-230](file://rag/vectorstore.py#L1-L230)
...
## 核心组件
- VectorStore：ChromaDB 向量库管理器，负责客户端延迟初始化、集合获取与创建、文档增删查、统计与清空、集合切换与列举。**新增智能批次处理机制，支持大规模文档批量入库**。
...
- [rag/vectorstore.py:47-67](file://rag/vectorstore.py#L47-L67)
...
**更新** 新增智能批次处理机制，支持大规模文档批量入库：
- 删除过程：使用5000个文档的批处理大小，避免删除操作超限
- 入库过程：使用4000个文档的批处理大小，避免ChromaDB默认批次限制(5461)
- 自动分批：通过循环分批处理，支持任意数量文档的批量入库
...
**更新** 新增智能批次处理机制：
- 批处理删除：使用5000个文档的批处理大小，避免删除操作超限
- 批处理入库：使用4000个文档的批处理大小，避免ChromaDB默认批次限制(5461)
- 进度跟踪：每批处理完成后记录调试日志，显示入库进度
...
BuildLists --> BatchDel["分批删除已存在文档<br/>(5000个/批)"]
BatchDel --> BatchAdd["分批新增文档<br/>(4000个/批)"]
BatchAdd --> LogProgress["记录入库进度日志"]
LogProgress --> ReturnCount["返回新增/覆盖数量"]
```

**图表来源**
- [rag/vectorstore.py:97-133](file://rag/vectorstore.py#L97-L133)
...
- [rag/chain.py:250-312](file://rag/chain.py#L250-L312)
- [rag/vectorstore.py:69-69](file://rag/vectorstore.py#L69-L69)
...
**章节来源**
- [rag/vectorstore.py:40-67](file://rag/vectorstore.py#L40-L67)
- [rag/vectorstore.py:97-133](file://rag/vectorstore.py#L97-L133)
- [rag/vectorstore.py:192-201](file://rag/vectorstore.py#L192-L201)
...
**章节来源**
- [rag/loader.py:22-259](file://rag/loader.py#L22-L259)
- [rag/vectorstore.py:135-143](file://rag/vectorstore.py#L135-L143)
...
- [rag/config.py:103-185](file://rag/config.py#L103-L185)
- [rag/vectorstore.py:202-230](file://rag/vectorstore.py#L202-L230)
- [scripts/ingest.py:25-66](file://scripts/ingest.py#L25-L66)
...
- **更新** 批量处理性能优化
  - 智能批次处理：使用4000个文档的批处理大小，平衡内存使用与处理效率
  - 分批删除：使用5000个文档的批处理大小，避免删除操作超限
  - 进度跟踪：详细的日志记录帮助监控大批量操作的执行状态
...
- 常见问题与处理
  ...
  - **更新** 批量处理问题：
    - 批量删除失败：检查是否有超过5000个文档需要删除，系统会自动分批处理
    - 批量入库失败：检查是否有超过4000个文档需要入库，系统会自动分批处理
    - 内存不足：大批量操作时注意监控内存使用，适当调整批处理大小
...
**更新** 新的智能批次处理机制进一步增强了系统的可扩展性和稳定性，能够高效处理大量文档的批量入库需求，同时保持系统的响应性和可靠性。
...
- **更新** 批量处理优化：
  - 批处理大小：默认使用4000个文档的批处理大小，可根据硬件配置调整
  - 内存监控：大批量操作时监控内存使用，避免内存溢出
  - 错误处理：分批处理提供了更好的错误隔离，单批失败不影响其他批次
