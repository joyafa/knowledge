...
## 更新摘要
**所做更改**
- 新增 PyInstaller 钩子支持章节，包含 torch.classes 和 chromadb 运行时钩子
- 更新构建系统架构，展示改进的打包流程
- 新增运行时钩子与依赖修复机制说明
...
**更新** 新增 PyInstaller + NSIS 构建系统和 PyInstaller 钩子支持
...
Hooks["PyInstaller 钩子<br/>hook-torch.classes.py<br/>runtime_hook_chromadb.py"]
...
**图表来源**
- [run_app.py:1-321](file://run_app.py#L1-L321)
- [knowledge_app.spec:1-218](file://knowledge_app.spec#L1-L218)
...
- **PyInstaller 钩子系统**：包含 torch.classes 排除钩子和 chromadb 运行时钩子，解决 C++ 扩展命名空间和动态导入问题。
...
## 架构总览
系统以 Streamlit 作为前端入口，通过服务层调用 RAG 检索引擎，检索引擎依赖配置中心与日志模块，最终与向量库进行交互。PyInstaller + NSIS 构建系统提供现代化的打包和安装解决方案，包含专门的钩子系统来处理复杂的依赖关系，docker-compose 将应用容器与可选的本地推理服务进行网络隔离与资源编排。

**更新** 新增 PyInstaller 钩子系统架构
...
participant Hooks as "PyInstaller 钩子<br/>hook-torch.classes.py<br/>runtime_hook_chromadb.py"
...
PyI->>Hooks : 应用钩子修复
Hooks-->>PyI : 修复 C++ 扩展命名空间
Hooks-->>PyI : 修复 chromadb 动态导入
PyI->>PyI : 分析依赖、收集数据文件
PyI->>PyI : 生成独立可执行文件
...
BuildPyI --> ApplyHooks["应用 PyInstaller 钩子<br/>修复依赖问题"]
ApplyHooks --> FixData["修复数据文件位置<br/>_internal 目录处理"]
...
**更新** 新增 PyInstaller 钩子支持章节

- **torch.classes 排除钩子**：hook-torch.classes.py 专门处理 PyInstaller 无法分析的 C++ 扩展命名空间，避免警告并减小安装包体积。
...
```mermaid
flowchart TD
Hooks["PyInstaller 钩子系统"] --> TorchHook["torch.classes 排除钩子<br/>hook-torch.classes.py"]
Hooks --> ChromaHook["chromadb 运行时钩子<br/>runtime_hook_chromadb.py"]
TorchHook --> ExcludeImports["排除 C++ 扩展命名空间"]
...
**章节来源**
- [run_app.py:174-321](file://run_app.py#L174-L321)
...
- **PyInstaller 钩子依赖**：torch.classes 排除钩子和 chromadb 运行时钩子。
...
Spec --> Hooks["PyInstaller 钩子<br/>hook-torch.classes.py<br/>runtime_hook_chromadb.py"]
Hooks --> Exe["独立可执行文件"]
Exe --> NSIS["NSIS 安装器"]
NSIS --> Installer["安装包"]
...
- **构建性能优化**
  ...
  - **钩子优化**：torch.classes 排除钩子避免不必要的分析开销。
...
- **构建系统问题**
  ...
  - **torch.classes 警告**：确认 hook-torch.classes.py 已正确应用排除钩子。
...
## 结论
本指南提供了从容器化部署到日常运维的完整方案。通过 **PyInstaller + NSIS 现代化构建系统**、**PyInstaller 钩子支持**、**run_app.py 统一入口点**、docker-compose 快速编排、配置中心灵活覆盖、向量库自动化入库与清理、健康检查与审计日志，以及速率限制与分析仪表盘，系统可在生产环境中稳定运行并持续优化。

**更新** 新增现代化构建系统和钩子支持带来的改进
...
- [run_app.py:1-321](file://run_app.py#L1-321)
- [knowledge_app.spec:1-218](file://knowledge_app.spec#L1-218)
- [scripts/installer/windows/knowledge_setup.nsi:1-141](file://scripts/installer/windows/knowledge_setup.nsi#L1-141)

### B. PyInstaller 钩子支持与运行时修复
- **torch.classes 排除钩子**：hook-torch.classes.py 排除 C++ 扩展命名空间，避免 PyInstaller 警告。
...
- **构建优化**：使用 PyInstaller 的 excludes 列表减小安装包体积，优化 DLL 搜索路径，应用钩子修复复杂依赖。
...
- **torch.classes 排除**：hook-torch.classes.py 排除 C++ 扩展命名空间，避免 PyInstaller 警告。

**更新** 新增运行时钩子说明和 torch.classes 排除钩子
