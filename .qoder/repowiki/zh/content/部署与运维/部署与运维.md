...
- [scripts/_download_wheels.py](file://scripts/_download_wheels.py)
...
## 更新摘要
**所做更改**
- 新增 PyInstaller torch 类别 hook 支持，完善 C++ 扩展命名空间处理
- 更新构建系统架构，展示增强的钩子系统配置
- 新增 torch 类别 hook 与现有 torch hook 的协同工作机制
- 更新 run_app.py 中的 DLL 搜索路径修复和模型检测功能
- 新增 Linux 离线安装包构建的 PyInstaller 集成
- 新增版本标准化和构建过程改进说明
- 新增离线部署打包脚本和下载依赖脚本
...
- **增强的 PyInstaller 钩子支持与 torch 类别处理**
- **run_app.py 统一入口点**
...
**更新** 新增增强的 PyInstaller + NSIS 构建系统和改进的钩子支持
...
Hooks["PyInstaller 钩子系统<br/>hook-torch.classes.py<br/>hook-torch.py<br/>runtime_hook_chromadb.py"]
NSIS["NSIS 安装器<br/>knowledge_setup.nsi"]
Build["构建脚本<br/>build_installer.py"]
Offline["离线部署<br/>prepare_offline.py"]
Wheels["依赖下载<br/>_download_wheels.py"]
end
subgraph "检索引擎层"
...
VDB["向量数据库<br/>./data/chroma_db"]
Hist["聊天历史<br/>./data/chat_history"]
Logs["系统日志<br/>./logs"]
end
subgraph "运行时"
...
**图表来源**
- [run_app.py:1-281](file://run_app.py#L1-L281)
...
- [scripts/_download_wheels.py:1-47](file://scripts/_download_wheels.py#L1-L47)
...
- **增强的 PyInstaller 钩子系统**：包含 torch.classes 排除钩子、torch 主钩子和 chromadb 运行时钩子，解决 C++ 扩展命名空间和动态导入问题。
- **NSIS 安装器**：knowledge_setup.nsi 提供 Windows 平台的图形化安装体验，支持 LLM 配置向导和一键安装。
- **离线部署工具**：prepare_offline.py 将项目目录打包为 zip，支持完全离线部署。
- **依赖下载工具**：_download_wheels.py 下载 Windows 平台所需的 wheel 依赖。
...
## 架构总览
系统以 Streamlit 作为前端入口，通过服务层调用 RAG 检索引擎，检索引擎依赖配置中心与日志模块，最终与向量库进行交互。增强的 PyInstaller + NSIS 构建系统提供现代化的打包和安装解决方案，包含改进的钩子系统来处理复杂的依赖关系，docker-compose 将应用容器与可选的本地推理服务进行网络隔离与资源编排。

**更新** 新增增强的 PyInstaller 钩子系统架构
...
participant TorchHooks as "torch 钩子系统<br/>hook-torch.py<br/>hook-torch.classes.py"
participant ChromaHook as "chromadb 钩子<br/>runtime_hook_chromadb.py"
participant NSIS as "NSIS 安装器<br/>knowledge_setup.nsi"
participant Offline as "离线部署<br/>prepare_offline.py"
participant User as "最终用户"
Dev->>Build : 运行构建脚本
Build->>PyI : 调用 PyInstaller
PyI->>TorchHooks : 应用 torch 钩子修复
TorchHooks-->>PyI : 修复 C++ 扩展命名空间
PyI->>ChromaHook : 应用 chromadb 钩子修复
ChromaHook-->>PyI : 修复动态导入问题
PyI->>PyI : 分析依赖、收集数据文件
PyI->>PyI : 生成独立可执行文件
PyI->>NSIS : 输出 staging 目录
NSIS->>NSIS : 生成安装包
NSIS-->>User : 发布安装包
Offline->>Offline : 打包离线部署包
Offline-->>User : 发布离线包
User->>User : 运行安装程序
User->>User : 配置 LLM 参数
User->>User : 启动应用
...
- [scripts/build_windows.py:54-171](file://scripts/build_windows.py#L54-L171)
- [scripts/hooks/hook-torch.classes.py:1-8](file://scripts/hooks/hook-torch.classes.py#L1-L8)
- [scripts/hooks/hook-torch.py:1-9](file://scripts/hooks/hook-torch.py#L1-L9)
- [scripts/hooks/runtime_hook_chromadb.py:1-50](file://scripts/hooks/runtime_hook_chromadb.py#L1-L50)
- [scripts/installer/windows/knowledge_setup.nsi:1-141](file://scripts/installer/windows/knowledge_setup.nsi#L1-L141)
- [scripts/prepare_offline.py:77-168](file://scripts/prepare_offline.py#L77-L168)
...
BuildPyI --> ApplyTorchHooks["应用 torch 钩子<br/>修复 C++ 扩展命名空间"]
ApplyTorchHooks --> ApplyChromaHook["应用 chromadb 钩子<br/>修复动态导入"]
ApplyChromaHook --> FixData["修复数据文件位置<br/>_internal 目录处理"]
...
- [scripts/hooks/hook-torch.py:1-9](file://scripts/hooks/hook-torch.py#L1-L9)
- [scripts/hooks/runtime_hook_chromadb.py:1-50](file://scripts/hooks/runtime_hook_chromadb.py#L1-L50)
- [scripts/installer/windows/knowledge_setup.nsi:137-171](file://scripts/installer/windows/knowledge_setup.nsi#L137-L171)
...
### 增强的 PyInstaller 钩子支持与运行时修复

**更新** 新增增强的 PyInstaller 钩子支持章节

- **torch 类别排除钩子**：hook-torch.classes.py 专门处理 PyInstaller 无法分析的 C++ 扩展命名空间，避免警告并减小安装包体积。
- **torch 主钩子**：hook-torch.py 提供 torch 主级别的排除配置，与 torch.classes 钩子协同工作，确保完整的 C++ 扩展处理。
- **chromadb 运行时钩子**：runtime_hook_chromadb.py 修复 chromadb 动态导入问题，通过 monkey-patch 拦截 pkgutil.iter_modules 来返回预置的子模块列表。
- **元数据补丁**：run_app.py 中的 _patch_importlib_metadata 函数处理 PyInstaller 打包后的 importlib.metadata 缺失问题。
- **DLL 搜索路径修复**：_patch_dll_search_path 函数修复 Windows DLL 加载失败问题，确保所有依赖库正确加载。

```mermaid
flowchart TD
EnhancedHooks["增强的 PyInstaller 钩子系统"] --> TorchClassesHook["torch.classes 排除钩子<br/>hook-torch.classes.py"]
EnhancedHooks --> TorchMainHook["torch 主钩子<br/>hook-torch.py"]
EnhancedHooks --> ChromaHook["chromadb 运行时钩子<br/>runtime_hook_chromadb.py"]
TorchClassesHook --> ExcludeImports["排除 C++ 扩展命名空间"]
TorchMainHook --> MainLevelExcludes["主级别排除配置"]
ChromaHook --> DynamicImport["修复动态导入<br/>拦截 pkgutil.iter_modules"]
RunApp["run_app.py"] --> MetaPatch["元数据补丁<br/>importlib.metadata.version"]
RunApp --> DLLPatch["DLL 搜索路径修复<br/>_patch_dll_search_path"]
RunApp --> ModelDetect["模型检测<br/>本地模型优先"]
```
...
**章节来源**
- [run_app.py:174-281](file://run_app.py#L174-L281)
...
- [scripts/prepare_linux_staging.py:20-158](file://scripts/prepare_linux_staging.py#L20-L158)

### 离线部署打包

**新增** 离线部署打包功能

- **完整打包**：prepare_offline.py 将项目目录（含虚拟环境和本地模型缓存）打包为 zip。
- **排除策略**：智能排除开发工具、缓存文件和版本控制目录。
- **部署说明**：自动生成离线部署说明文档，包含详细的部署步骤。
- **目标机器**：解压后即可运行，无需联网安装。
...
- **平台适配**：_download_wheels.py 下载 Windows 平台所需的 wheel 依赖。
- **二进制包处理**：区分二进制包和源码包，分别处理下载策略。
- **缓存管理**：将下载的 wheel 存储到 build_cache/wheels_windows 目录。
- **构建集成**：与构建系统无缝集成，支持离线构建。
...
**章节来源**
- [config.yaml:1-93](file://config.yaml#L1-L93)
...
- [config.yaml:82-83](file://config.yaml#L82-L83)
...
Spec --> TorchHooks["torch 钩子系统<br/>hook-torch.py<br/>hook-torch.classes.py"]
Spec --> ChromaHook["chromadb 钩子<br/>runtime_hook_chromadb.py"]
TorchHooks --> Exe["独立可执行文件"]
ChromaHook --> Exe
Exe --> NSIS["NSIS 安装器"]
NSIS --> Installer["安装包"]
Offline["离线部署<br/>prepare_offline.py"] --> Zip["Zip 打包"]
Zip --> OfflinePkg["离线部署包"]
Img --> App["应用代码(app.py)"]
App --> Cfg["配置中心(rag/config.py)"]
App --> Log["日志(rag/logging_config.py)"]
App --> VS["向量库(rag/vectorstore.py)"]
VS --> VDB["ChromaDB(./data/chroma_db)"]
...
- **构建性能优化**
  ...
  - **钩子优化**：torch.classes 排除钩子和 torch 主钩子避免不必要的分析开销。
  - **离线部署**：prepare_offline.py 支持完整离线部署，避免网络依赖。
- UI 与并发
...
**更新** 新增构建性能优化建议和增强的钩子优化

**章节来源**
- [config.yaml:4-93](file://config.yaml#L4-L93)
...
- [knowledge_app.spec:157-170](file://knowledge_app.spec#L157-L170)
- [run_app.py:218-235](file://run_app.py#L218-L235)
- [scripts/hooks/hook-torch.classes.py:1-8](file://scripts/hooks/hook-torch.classes.py#L1-L8)
- [scripts/hooks/hook-torch.py:1-9](file://scripts/hooks/hook-torch.py#L1-L9)
...
- **构建系统问题**
  ...
  - **torch 类别警告**：确认 hook-torch.classes.py 和 hook-torch.py 已正确应用排除钩子。
  - **离线部署失败**：检查 prepare_offline.py 的排除规则和打包完整性。
- 向量库异常
...
- [scripts/build_windows.py:62-76](file://scripts/build_windows.py#L62-L76)
- [scripts/installer/windows/knowledge_setup.nsi:141-148](file://scripts/installer/windows/knowledge_setup.nsi#L141-L148)
- [run_app.py:98-123](file://run_app.py#L98-L123)
- [scripts/hooks/runtime_hook_chromadb.py:37-47](file://scripts/hooks/runtime_hook_chromadb.py#L37-L47)
- [scripts/hooks/hook-torch.classes.py:1-8](file://scripts/hooks/hook-torch.classes.py#L1-L8)
- [scripts/hooks/hook-torch.py:1-9](file://scripts/hooks/hook-torch.py#L1-L9)
- [scripts/prepare_offline.py:47-75](file://scripts/prepare_offline.py#L47-L75)

## 结论
本指南提供了从容器化部署到日常运维的完整方案。通过 **增强的 PyInstaller + NSIS 现代化构建系统**、**改进的 PyInstaller 钩子支持**、**run_app.py 统一入口点**、docker-compose 快速编排、配置中心灵活覆盖、向量库自动化入库与清理、健康检查与审计日志，以及速率限制与分析仪表盘，系统可在生产环境中稳定运行并持续优化。

**更新** 新增现代化构建系统和增强的钩子支持带来的改进
...
### B. 增强的 PyInstaller 钩子支持与运行时修复
- **torch 类别排除钩子**：hook-torch.classes.py 排除 C++ 扩展命名空间，避免 PyInstaller 警告。
- **torch 主钩子**：hook-torch.py 提供 torch 主级别的排除配置，与 torch.classes 钩子协同工作。
- **chromadb 运行时钩子**：runtime_hook_chromadb.py 修复动态导入问题，确保所有 embedding function 子模块可用。
- **元数据补丁**：处理 importlib.metadata 缺失问题，避免版本检查失败。
- **DLL 搜索路径修复**：确保 Windows 环境下所有依赖库正确加载。
...
- **构建优化**：使用 PyInstaller 的 excludes 列表减小安装包体积，优化 DLL 搜索路径，应用增强的钩子修复复杂依赖。
- **版本标准化**：统一版本号管理，确保构建产物的一致性。

**更新** 新增构建优化最佳实践和增强的钩子优化
...
- **安装包升级**：使用新的增强的 PyInstaller + NSIS 构建系统生成的安装包进行升级。
- **版本标准化**：统一版本号管理，确保构建产物的一致性。
...
- **离线包备份**：备份 prepare_offline.py 生成的离线部署包。

**更新** 新增安装包备份建议和离线包备份

**章节来源**
- [scripts/prepare_offline.py:127-168](file://scripts/prepare_offline.py#L127-L168)

### N. 增强的运行时钩子与依赖修复
- **torch 钩子系统**：hook-torch.py 和 hook-torch.classes.py 共同处理 torch C++ 扩展命名空间。
