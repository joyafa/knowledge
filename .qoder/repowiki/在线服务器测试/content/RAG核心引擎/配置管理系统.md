...
## 更新摘要
**变更内容**
- 新增RerankerConfig类，支持Cross-Encoder重排序模型配置
- AppConfig类改进，新增model_root配置支持本地模型根目录
- UIConfig新增company_url字段，支持公司链接显示
- 配置文件结构更新，新增reranker配置部分
- RAG链路中集成重排序模型支持
...
run_app["run_app.py<br/>应用启动脚本"] --> cfg
```
...
- **新增**：RerankerConfig类提供重排序模型配置，AppConfig新增model_root支持本地模型根目录。
...
- **新增**：model_root支持环境变量替换，开发环境默认"./model"，打包后由安装脚本设置
...
- **新增**：model_root支持环境变量替换，安装脚本自动设置绝对路径
...
- **新增**：应用启动时设置MODEL_ROOT环境变量，确保相对路径正确解析
...
- **组件依赖**：
  ...
  - ui/login.py、ui/sidebar.py 依赖 rag/config.py 的UI配置（**新增company_url**）
...
**主要改进**：
- **重排序模型支持**：新增RerankerConfig类，显著提升检索质量
- **本地模型管理**：model_root配置支持离线部署，提高系统稳定性
...
- **新增**：合理配置model_root，确保离线部署时模型路径正确
...
- **embedding**
  - model：嵌入模型名称
  - local_path：本地模型路径（可选，相对model_root）
- **reranker**（**新增**）
  - model：重排序模型名称（默认"BAAI/bge-reranker-base"）
  - local_path：本地重排序模型路径（可选，相对model_root）
- **vectorstore**
