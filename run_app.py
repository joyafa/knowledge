...
def main():
    ...
    # 复用 rag/config.py 中的 detect_local_models()，避免重复逻辑
    model_dir = app_root / "model"

    from rag.config import detect_local_models
    detect_local_models(str(model_dir))

    has_local_emb = bool(os.environ.get("EMBEDDING_LOCAL_PATH"))
    has_local_reranker = bool(os.environ.get("RERANKER_LOCAL_PATH"))

    if has_local_emb:
        print(f"  [MODEL] 检测到本地 Embedding 模型: {os.environ['EMBEDDING_LOCAL_PATH']}")
    else:
        print(f"  [MODEL] 未检测到本地 Embedding 模型，首次运行将自动下载")

    if has_local_reranker:
        print(f"  [MODEL] 检测到本地 Reranker 模型: {os.environ['RERANKER_LOCAL_PATH']}")
    else:
